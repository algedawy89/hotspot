...
function onLoginSubmitted() {
    var a = document.getElementById("error");
		...
		1 > document.login.username.value.length ? (a.innerText = "يجب ادخال قيمة صحيحة!", showErrorPopup(), animating = !1) : (a.innerText = "", $loginSubmit.classList.add("processing"), userLoginH())
		} else {
		1 > document.login.username.value.length ? (a.innerText = "يجب ادخال قيمة صحيحة!", showErrorPopup(), animating = !1) : (a.innerText = "", $loginSubmit.classList.add("processing"), 5 > currentSpeedClass.length ? (currentSpeedClass = document.querySelector("input[name='domain'][checked]").value, showParentElement($loginSubmit, document.getElementById("chose-speed"))) : userLogin())
...
function showLoginPage() {
    loggedIn ? showStatusPage() : ($login.style.visibility = "visible", $login.style.display = "block", getComputedStyle($login).top, $login.classList.remove("inactive"))
}

function showStatusPage() {
    showParentElement(null, $status)
}
...
function initHotspotEvents(a, b) {
    ...
    a.addEventListener("click", function(f) {
        animating || (animating = !0, "status" === b.id ? onLoginSubmitted() : ("app-store" === b.id && loadScript("js/store.min.js"), "magazine" === b.id && loadScript("js/magazine.min.js"), this.classList.add("processing"), showParentElement(this, b)))
    });
    for (d in c) {
        var e = c[d];
        "object" == typeof e && e.addEventListener("click", function(f) {
            if (!animating) {
                ...
                setTimeout(function() {
                    ...
                    "magazine-player" === b.id ? (showParentElement(null, document.getElementById("magazine")), document.getElementById("player").innerHTML = "") : showLoginPage()
                }, submitPhase2 + 120);
                setTimeout(function() {
...
function onLoginStart(a) {
    ...
    setTimeout(function() {
        return document.login.username.focus()
    }, 50)
}
...
function onLoginError(a) {
    ...
    animating = !1;
    if (!a.error.includes("already authorizing, retry later") && (document.getElementById("error").innerText = toArabicError(a.error), showLoginPage(), showErrorPopup(), 1 === hotspotConfig["enable-hot-blocker"])) {
...
function onLoggedIn(a) {
    ...
    null != a && (resetCounter(), startStatusQuery())
}
        ...
        b.onreadystatechange = function() {
            if (4 === this.readyState) {
                ...
                try {
                    ...
                    loggedIn = 0, "/status?var=callBack" === a && hideStatusPage(), showLoginPage()
                }
            }
        };
        b.send()
...
function userLoginH() {
    ...
    "" !== c && (c = c.value);
    a = "/login?username=".concat(a, "&password=").concat(c, "&var=callBack");
    getRequest(a)
}
...
function userLogin() {
    var a = document.login.querySelector("input[username-field]").value,
        b, c = null != (b = document.login.querySelector("input[password-field]")) ? b : "";
    "" !== c && (c = c.value);
    a = "/login?username=".concat(a, "&password=").concat(c.split("@wifa-man").join(""), "&var=callBack").concat("&domain=", currentSpeedClass);
    getRequest(a)
}
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    a = document.querySelectorAll("input[name='domain']");
    for (b in a)
        if (c = a[b], "object" == typeof c) {
        ...
        } var e = document.getElementById("chose-speed");
    document.getElementById("speed_chosen").addEventListener("click", function() {
        e.style.display = "none";
        e.classList.add("inactive")
    });
    document.login.addEventListener("submit", function(f) {
        animating || onLoginSubmitted();
        f.preventDefault()
    });
    setTimeout(cookieLogin, 200)
}, !1);